...
@NullMarked
public class BedwarsMapInstance {
    ...
    public CompletableFuture<World> loadMap() {
        return copyWorldFiles()
            .thenApplyAsync(finished -> {
                this.world = Bukkit.createWorld(new WorldCreator(worldName()).generator(new EmptyChunkGenerator()));
    ...
    public CompletableFuture<Void> deleteMapAsync() {
        return CompletableFuture.runAsync(() -> {
            ...
            try {
                ...
                BedwarsPaper.logger().warn("An error occurred whilst trying to delete {}-{}", bedwarsMap.name(), id, directoryRemoveException);
            }
        });
    }
